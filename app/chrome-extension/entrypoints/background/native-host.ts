...
export function connectNativeHost(port: number = NATIVE_HOST.DEFAULT_PORT): boolean {
  ...
  try {
    ...
    // Listener is intentionally NOT async — tool calls use fire-and-forget
    // so that multiple calls targeting different tabs execute in parallel.
    // The per-tab queue in handleCallTool serializes same-tab operations.
    nativePort.onMessage.addListener((message) => {
      if (message.type === NativeMessageType.PROCESS_DATA && message.requestId) {
        ...
        const requestId = message.requestId;
        // Fire-and-forget: returns immediately so the next message can be processed
        handleCallTool(message.payload)
          .then((result) => {
            nativePort?.postMessage({
              responseToRequestId: requestId,
              payload: {
        ...
        const requestId = message.requestId;
        // Fire-and-forget for flow listing
        (async () => {
          try {
        ...
        saveServerStatus(currentServerStatus).catch(() => {});
        broadcastServerStatusChange(currentServerStatus);
