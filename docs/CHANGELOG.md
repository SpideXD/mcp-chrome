...
## [v3.0.0] - The Multiplier

### Added

- **Multi-Tenant Tab Multiplexing**: Re-architected native server messaging to allow multiple AI agents to connect concurrently.
- **Tab Context Isolation**: Introduced `TabExecutionQueue` allowing independent agents to execute tools in parallel on different tabs.
- **Execution Queue Serialization**: Automatically serializes commands targeting the same tab ID to prevent DOM/CDP race conditions between overlapping operations.
